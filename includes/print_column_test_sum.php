<?php
	...
	// ----------------------------------------------------------------------------------
	function print_column_test_sum($col, $sum)
	{
		...
		$temp_table = 'temp_column_sum_' . $sum . '_' . $col; 
		...
		/*
		//start table
		print("<h3>Column $col-$sum Count</h3>\n");
		...
		*/
		...
		$column_sum_count_array = array_fill (0,$balls+1,$temp_array);
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			if ($game == 10 or $game == 20)
			{
				...
				while($row2 = mysqli_fetch_array($mysqli_result2))
				{
					...
					for ($d = $z; $d <= 27; $d++) {$column_sum_count_array_aon[$y][$d]++;}
					$z++;
				}
					
			} elseif ($draw_date_unix == $day1_unix) { 
				for ($d = 0; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $week1_unix) {
				for ($d = 1; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $week2_unix) {
				for ($d = 2; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $month1_unix) {
				for ($d = 3; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $month3_unix) {
				for ($d = 4; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $month6_unix) {
				for ($d = 5; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year1_unix) {
				for ($d = 6; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year2_unix) {
				for ($d = 7; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year3_unix) {
				for ($d = 8; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year4_unix) {
				for ($d = 9; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year5_unix) {
				for ($d = 10; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year6_unix) {
				for ($d = 11; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			#} elseif ($draw_date_unix > $year6_unix) {
			#	for ($d = 11; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year7_unix) {
				for ($d = 12; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year8_unix) {
				for ($d = 13; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year9_unix) {
				for ($d = 14; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			} elseif ($draw_date_unix > $year10_unix) {
				for ($d = 15; $d <= 15; $d++) {$column_sum_count_array[$x][$d]++;}
			}
	
			$column_sum_count_array[$x][16]++;

			#add 1 year to clear
			if ($first_draw_unix > $year7_unix) {
				for ($d = 13; $d <= 15; $d++) {$column_sum_count_array[$x][$d]=0;}
			} elseif ($first_draw_unix > $year8_unix) {
				for ($d = 14; $d <= 15; $d++) {$column_sum_count_array[$x][$d]=0;}
			} elseif ($first_draw_unix > $year9_unix) {
				for ($d = 15; $d <= 15; $d++) {$column_sum_count_array[$x][$d]=0;}
			} elseif ($first_draw_unix > $year10_unix) {
				for ($d = 16; $d <= 16; $d++) {$column_sum_count_array[$x][$d]=0;}
			}
		...
		{	
			...
			if ($game == 10 or $game == 20)
			{
				for ($d = 1; $d <= 15; $d++)
				{
					if ($column_sum_count_array[$x][$d] > 30)
					{
						print("<TD bgcolor=\"#FF0033\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} elseif ($column_sum_count_array[$x][$d] > 5) {
						print("<TD bgcolor=\"#CCFFFF\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} elseif ($column_sum_count_array[$x][$d] > 1) {
						print("<TD bgcolor=\"#CCFF66\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} elseif ($column_sum_count_array[$x][$d] == 1) {
						print("<TD bgcolor=\"#F1F1F1\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} else {
						print("<TD align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					}
				}
			} else {
				for ($d = 0; $d <= 15; $d++)
				{
					if ($column_sum_count_array[$x][$d] > 79)
					{
						print("<TD bgcolor=\"#FF0033\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} elseif ($column_sum_count_array[$x][$d] > 15) {
						print("<TD bgcolor=\"#CCFFFF\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} elseif ($column_sum_count_array[$x][$d] > 1) {
						print("<TD bgcolor=\"#CCFF66\" align=center>{$column_sum_count_array[$x][$d]}</TD>\n");
					} elseif ($column_sum_count_array[$x][$d] == 1) {
			...
			if ($column_sum_count_array[$x][16] > 79)
			{
				print("<TD bgcolor=\"#FF0000\" align=center><font color=\"#ffffff\"><b>{$column_sum_count_array[$x][16]}</b></font></TD>\n");
			} else {
				print("<TD align=center>{$column_sum_count_array[$x][16]}</TD>\n");
			}
			...
			for ($d = 0; $d <= 15; $d++)
			{
				$sigma_sum += $column_sum_count_array[$x][$d];
			}

			#print("<TD align=center>$sigma_sum</TD>\n");

			$sum_temp_y1 = number_format(($column_sum_count_array[$x][6]/365),3);
			...
			$sum_temp_y5 = number_format(($column_sum_count_array[$x][10]/(365*5)),3);
			...
			*/
			...
			$weighted_average = (
				#($column_sum_count_array[$x][1]/7*100*0.05) + #week1
				#($column_sum_count_array[$x][3]/30*100*0.05) + #month1
				#($column_sum_count_array[$x][5]/(365/2)*100*0.20) + #month6
				($column_sum_count_array[$x][6]/365*100*0.33) + #year1
				($column_sum_count_array[$x][8]/(365*3)*100*0.33) + #year3
				($column_sum_count_array[$x][10]/(365*5)*100*0.34)); #year5

			$sum_temp = number_format($weighted_average,3);

			#print("x = $x<br>");

			#print_r ("$column_sum_count_array");
			#die ("$column_sum_count_array");

			$query5 = "INSERT INTO $temp_table ";
			$query5 .= "VALUES ('$x', ";
			for ($d = 0; $d <= 15; $d++) 
			{
				$query5 .= "'{$column_sum_count_array[$x][$d]}', ";
			}
			#$query5 .= "'$num_rows', ";
			$query5 .= "'{$column_sum_count_array[$x][16]}', ";
			$query5 .= "'$sum_temp_y1', ";
			$query5 .= "'$sum_temp_y5', ";
		...
		### start sorted table ####################################################################
		print("<h3>Column $col-$sum Count - Sort by percent_wa</h3>\n");
		print("<TABLE BORDER=\"1\">\n");
