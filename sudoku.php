...
function is_possible_row($number,$row,$sudoku){
	$possible = true;
	for($x=0;$x<=8;$x++){		
		#echo "{$sudoku[$row*9+$x]} == $number<br>";
		if($sudoku[$row*9+$x] == $number){
			$possible = false;
			#echo "possible = false<br>";
		}		
	}
	return $possible;
}
...
function is_possible_number($cell,$number,$sudoku){
	...
	echo "is_possible_number == row = $row, col = $col, block = $block<br>";
...
function is_solved_sudoku($sudoku){
	for($x=0;$x<=8;$x++){
		if(!is_correct_block($x,$sudoku) or !is_correct_row($x,$sudoku) or !is_correct_col($x,$sudoku)){
			echo "is_solved_sudoku - false<p>";
			return false;
			break;
		}
	}
	echo "is_solved_sudoku - true<p>";
	return true;
}
...
function determine_possible_values($cell,$sudoku){
	...
	for($x=1;$x<=9;$x++){
		if(is_possible_number($cell,$x,$sudoku)){
			...
			echo "determine_possible_values - unshift<br>";
		} 
	}
	return $possible;
}
...
function scan_sudoku_for_unique($sudoku){
	...
	print_r ($possible);
	echo "<br>";
	echo "scan_sudoku_for_unique return $possible<p>";
...
function remove_attempt($attempt_array,$number){
	...
	echo "remove_attempt preformed<p>";
	return $new_array;
}
...
function solve($sudoku){
	...
	while(!is_solved_sudoku($sudoku)){
		...
		echo "next_move = $next_move in solve<br>";
		if($next_move == false){
			$next_move = array_pop($saved);
			$sudoku = array_pop($saved_sud);
		}
		$what_to_try = next_random($next_move);	
		echo "what_to_try = $what_to_try in solve<br>";
		$attempt = determine_random_possible_value($next_move,$what_to_try);
		echo "attempt = $attempt in determine_random_possible_value<br>";
		$temp = count($next_move[$what_to_try]);
		echo "count next_move[what to try] = $temp<br>";
